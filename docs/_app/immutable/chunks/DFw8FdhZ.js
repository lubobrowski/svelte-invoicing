								...
								return (
									...
									i
										.map((c, l) => {
											if (l % 2) {
												if (c.startsWith('x+'))
													return Tt(String.fromCharCode(parseInt(c.slice(2), 16)))
												if (c.startsWith('u+'))
													return Tt(
														String.fromCharCode(
															...c
...
async function Kn(t, e, n) {
	...
	r && ((history.scrollRestoration = 'manual'), scrollTo(r.x, r.y)),
		n
			? await Nn(Nt, n)
			: await xn(S.hash ? $n(new URL(location.href)) : location.href, { replaceState: !0 }),
		On()
}
function kn() {
	;(wt.length = 0), (Gt = !1)
		...
		const b = v.branch[y],
			E =
				...
				((b == null ? void 0 : b.loader) !== g[1] ||
					se(m, w, u, f, (D = b.server) == null ? void 0 : D.uses, r))
		return E && (m = !0), E
	})
	if (p.some(Boolean)) {
		try {
	...
	const R = l.map(async (g, y) => {
		...
		return Kt({
			...
			parent: async () => {
				var Jt
				const zt = {}
				for (let It = 0; It < y; It += 1)
					Object.assign(zt, (Jt = await R[It]) == null ? void 0 : Jt.data)
				return zt
			},
			server_data_node: Wt(
...
async function Ln(t) {
	...
	try {
		const r = (async () => {
			let a =
				(await S.hooks.reroute({ url: new URL(t), fetch: async (s, i) => Ie(s, i, t).promise })) ??
				t
			if (typeof a == 'string') {
				const s = new URL(t)
...
function Tn(t) {
	return (
		Fe(S.hash ? t.hash.replace(/^#/, '').replace(/[?#].+/, '') : t.pathname.slice(x.length)) || '/'
	)
}
function bt(t) {
...
async function dt({
	...
	if (((e = (w == null ? void 0 : w.url) || e), V !== c))
	...
	if (((L = null), (h.props.page.state = i), _t)) {
		...
		const g = (await Promise.all(Array.from(En, (y) => y(f.navigation)))).filter(
			(y) => typeof y == 'function'
		)
		if (g.length > 0) {
			let y = function () {
	...
	if (ae) {
		const g =
			...
			document.getElementById(
				decodeURIComponent(S.hash ? (e.hash.split('#')[2] ?? '') : e.hash.slice(1))
			)
...
function Pn() {
	...
	function i() {
		...
		for (const o of j.querySelectorAll('a')) {
			...
			const u = pt(o)
			u.reload ||
				(u.preload_code === F.viewport && a.observe(o), u.preload_code === F.eager && xt(c))
		}
	}
	z.add(i), i()
}
...
function xn(t, e = {}) {
	return (
		(t = new URL(ge(t))),
		t.origin !== ut ? Promise.reject(new Error('goto: invalid URL')) : Ht(t, e, 0)
	)
}
function Cn(t) {
...
function On() {
	...
	;(history.scrollRestoration = 'manual'),
		...
		addEventListener('hashchange', () => {
			W &&
				((W = !1), history.replaceState({ ...history.state, [G]: ++k, [X]: T }, '', location.href))
		})
...
async function Nn(
	...
	let d
	;({ params: a = {}, route: s = { id: null } } = (await kt(l, !1)) || {}),
		(d = Bt.find(({ id: f }) => f === s.id))
	let u,
		w = !0
	try {
					...
					(w = h.nodes) == null ||
						w.forEach((_) => {
							;(_ == null ? void 0 : _.type) === 'data' &&
								((_.uses = xe(_.uses)), (_.data = d(_.data)))
						}),
						i(h)
